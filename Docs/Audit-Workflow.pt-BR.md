...
3. **Localize dependências.** O comando principal usa o CSV do levantamento em `C:\scriptsDC` e descobre os servidores no domínio atual pelo AD. Para limitar o escopo, `-ComputerName` continua disponível como opção avançada. Inclua processos apenas quando necessário. Examine a situação da coleta antes das correspondências e revise manualmente o inventário sem correspondência, grupos administrativos locais e grupos usados por tarefas.
...
A execução padrão é `.\Start-Audit.ps1`, sem parâmetros. O domínio vem da máquina local; servidores Windows habilitados e DCs são descobertos pelo AD. Os resultados ficam em `C:\scriptsDC`; relatórios anteriores são arquivados em `History` pelo comando principal. Confira `RunStatus.csv` e os relatórios detalhados. O escopo automático é o domínio atual, não toda a floresta nem máquinas fora do AD.
