...
3. **Locate dependencies.** The entry point uses the discovery CSV in `C:\scriptsDC` and discovers servers through the current AD domain. To limit scope, `-ComputerName` remains an advanced option. Add process collection only when needed. Inspect the scan status before the matched report, then review unmatched inventory, local administrative groups and task group principals manually.
...
The default command is `.\Start-Audit.ps1` with no parameters. The local computer supplies the domain; enabled Windows servers and DCs are discovered through AD. Reports go to `C:\scriptsDC`; the entry point archives previous reports under `History`. Review `RunStatus.csv` and detailed coverage. Automatic scope is the current domain, not the entire forest or machines outside AD.
